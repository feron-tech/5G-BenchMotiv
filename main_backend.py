...
class Backend:
	...
	def run_exp(self):
		...
		if (self.df_in_user['in_app_base'].iloc[0]):
			...
			try:
				subprocess.run(["python", "physical.py"], capture_output=True, text=True)
	...
	def get_baseline_measurements(self):
		try:
			...
			try:
				...
				ping_packs = int(self.df_in_user['ping_packs'].iloc[0])
			...
			mystr=str(_camp_name)+gparams._DELIMITER+\
				str(self.counter_camp)+gparams._DELIMITER+\
			      ...
			      str(base_dict['iperf_udp_ul_lost_percent'][0])
